...
fun TaskEditorScreen(
    ...
    // Subtasks (simple checkboxes shown under a task)
    data class SubtaskDraft(val id: String, var title: String, val sortOrder: Int)
    val subtasks = remember(existing?.id) {
        mutableStateListOf<SubtaskDraft>().apply {
            existing?.subtasksList?.forEachIndexed { idx, st ->
                add(SubtaskDraft(id = st.id, title = st.title, sortOrder = st.sortOrder.takeIf { it != 0 } ?: idx))
            }
        }
    }
    ...
    Scaffold(
        ...
        Column(
            ...
            if (subtasks.isEmpty()) {
                Text(
                    "Add optional checklist items for this task.",
            ...
            subtasks.forEachIndexed { index, s ->
                Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
                    OutlinedTextField(
                        value = s.title,
                        onValueChange = { new -> subtasks[index] = s.copy(title = new) },
                        modifier = Modifier.weight(1f),
            ...
            OutlinedButton(
                onClick = { subtasks.add(SubtaskDraft(UUID.randomUUID().toString(), "")) },
