...
fun TaskEditorScreen(
    ...
    data class UiSubtask(val id: String, val title: String)

    val subtasks = remember {
        mutableStateListOf<UiSubtask>().apply {
            existing?.subtasksList?.forEach { add(UiSubtask(it.id, it.title)) }
        }
    }
                ...
                .apply {
                    subtasks.forEach { s ->
                        if (s.title.trim().isNotEmpty()) {
                            addSubtasks(com.reachravi55.mydailyroutine.Subtask.newBuilder()
                                .setId(s.id)
    ...
    Scaffold(
        ...
        Column(
            ...
            if (lists.isEmpty()) {
                Card(Modifier.fillMaxWidth()) {
                    Column(Modifier.padding(14.dp)) {
                        ...
                        Text("Create a list first, then save tasks into it.", color = MaterialTheme.colorScheme.onSurfaceVariant)
                        Spacer(Modifier.height(10.dp))
                        Button(onClick = { showCreateListDialog = true }) { Text("Create list") }
                    }
                }
            } else {
                ListPicker(
            ...
            if (subtasks.isEmpty()) {
                Text("Optional: add smaller checkboxes under this task.", color = MaterialTheme.colorScheme.onSurfaceVariant)
                Spacer(Modifier.height(8.dp))
            }
                ...
                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    OutlinedTextField(
                        ...
                        onValueChange = { newTitle ->
                            subtasks[idx] = s.copy(title = newTitle)
                        },
                        modifier = Modifier.weight(1f),
                        label = { Text("Subtask") },
                        singleLine = true
                    )
                    IconButton(onClick = { subtasks.removeAt(idx) }) {
            ...
            OutlinedButton(
                onClick = { subtasks.add(UiSubtask(id = Ids.id(), title = "")) },
                modifier = Modifier.fillMaxWidth()
            ) {
