    ...
    suspend fun ensureInitialized() {
        store.updateData { current ->
            ...
            if (!current.hasSettings()) {
                b.setSettings(Settings.newBuilder().setNotificationsEnabled(true))
            }

            // First-run UX: always ensure there is at least one list so tasks can be saved.
            if (current.listsCount == 0) {
                val defaultListId = "inbox-${UUID.randomUUID()}"
                val inbox = RoutineList.newBuilder()
                    .setId(defaultListId)
                    .setName("Inbox")
                    .setSortOrder(1)
                    .build()
                b.addLists(inbox)
                b.activeListId = defaultListId
            } else if (current.activeListId.isBlank()) {
                // If lists exist but none is active, pick the first one.
                b.activeListId = current.listsList.first().id
            }
