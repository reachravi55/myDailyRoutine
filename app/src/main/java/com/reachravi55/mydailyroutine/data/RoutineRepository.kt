    ...
    suspend fun ensureInitialized() {
        store.updateData { current ->
            ...
            if (!current.hasSettings()) {
                b.settings = Settings.newBuilder()
                    .setNotificationsEnabled(true)
                    .build()
            }

            // Ensure we always have at least one list
            if (current.listsCount == 0) {
                val defaultId = Ids.id()
                val list = RoutineList.newBuilder()
                    ...
                    .build()
                b.addLists(list)
                b.activeListId = defaultId
            } else if (current.activeListId.isBlank()) {
                b.activeListId = current.listsList[0].id
            }
